...
function displayPics() {
  ...
  // The reason the line "console.log(rando);" threw an error is because the variable rando was declared within a function, meaning that the variable was only operable within the function. The line in question existed outside the function, meaning that the program did not recognize rando as a declared variable in that context. However, even if the original declaration was not encapsulated by a function, the same error still would have been thrown because using 'const' block-scopes the declared variable within proximate {}.
  console.log(viewed);
